...
class MockedDataCRUDTestCase(APITestCase):
    ...
    def test_crud_employees_from_mocked_data(self):
        ...
        # test_percentage_of_employees_per_industry
        response = self.client.get(
            reverse("percentage-employees-per-industry")
            )
        self.assertEqual(response.status_code, status.HTTP_200_OK)

        data = response.json()
        major_pharmaceuticals_percentage = next(
            item
            for item in data
            if item["industry"]
        ...
        self.assertAlmostEqual(
            major_pharmaceuticals_percentage["percentage"],
            5.2
            )
